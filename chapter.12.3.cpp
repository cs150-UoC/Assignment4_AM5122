    ...
    ImageData(string ppmname) {
        ...
        for(int i = 0; i<=height-1; i++) {
            ...
            for(int j = 0; j<=width-1; j++) {
                ...
                istringstream ss(line);
                ss >> r >> g >> b;
                Image[i].push_back(Pixel((j+1) * 20 - 10,(i+1) * 20 - 10,vec3(r,g,b)));
            }
        }
        fileppm.close();
    }
...
int main()
{
    ...
    for(int i = 0; i<=ppmimage.getheight() - 1; i++) {
        for(int j = 0; j<=ppmimage.getwidth() - 1; j++) {
            Fl_Color c = fl_rgb_color(uchar(Image[i][j].getcolor().x()),uchar(Image[i][j].getcolor().y()),uchar(Image[i][j].getcolor().z()));
